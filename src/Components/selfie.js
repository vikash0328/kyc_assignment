function drawvideo(video,context,width,height){
    ...
    context.scale(0.95,.9);
     ...
     context.strokeStyle = 'blue';  // some color/style
      context.lineWidth = 2;     
  context.arc(width/2, height/2,Math.pow((width*width/8)+(height*height/8),1/2) , 0, Math.PI*2, false);
  context.stroke(); 
  context.closePath();
  context.restore();
  ...
  class WebCam
  {
    ...
    async SetUp(){
      return new Promise((resolve,reject)=>{
        ...
        {
           navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:'user',  width:{min: 720, ideal: 1280, max: 1080}, height: { min: 200, ideal: 480, max:720  }}}).then((MediaStream)=>{
             ...
             this.CameraElement.addEventListener(
               'loadeddata', async()=>{
                 ...
                 this.SetAspectRatio(this.CameraElement.width,this.CameraElement.height);
  ...
  //context.restore();
    ...
    setup2()
    {
                ...
                this.CameraElement.addEventListener('play' ,async=> {
                  console.log(this.context);
                  var self=this;
                  drawvideo(self.CameraElement,self.CanvasCameraElement,self.CameraElement.width,self.CameraElement.height);
